...
exports.getStudentListPage = async (ctx, next) => {
  ...
  const studentList = await databasetool.findList("t_student", {
    name: keyword
  });
...
exports.addStudent = async (ctx, next) => {
  ...
  const result = await databasetool.insertOne("t_student", ctx.req.body);
  ctx.type = "html";
  if (result == null) {
    // 新增失败
...
exports.getEditStudentPage = async (ctx, next) => {
  const result = await databasetool.findOne("t_student", {
...
exports.deleteStudent = async (ctx, next) => {
  const result = await databasetool.deleteOne("t_student", {
    id: ctx.params.studentId
  });
